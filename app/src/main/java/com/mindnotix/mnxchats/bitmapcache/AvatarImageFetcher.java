...
public class AvatarImageFetcher {
	private static final String TAG ="AvatarImageFetcher" ;
	...
	public static AvatarImageFetcher getAvatarImageFetcher(Activity activity) {
		...
		imageFetcher.setLoadingImage(R.drawable.ic_def_profile);
	...
	public void loadImage(String jid, CircleImageView imageView) {
		if (jid == null) {
			return;
		}
	...
	public class BitmapWorkerTask extends AsyncTask<String, Void, Drawable> {
		...
		public BitmapWorkerTask(String jid, CircleImageView imageView) {
			this.jid = jid;
		...
		protected Drawable doInBackground(String... params) {
			...
			if (bitmap == null && !isCancelled() && getAttachedImageView() != null) {
				...
				try {
					user = OpenfireService.search(jid);
				} catch (Exception e) {
